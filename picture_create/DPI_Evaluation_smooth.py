...
def setSubPlot(scores, ylabel, label, title, linestyle):
    ...
    scores_smooth = make_interp_spline(x, scores)(x_smooth)
    plt.xlabel("数据包个数")
    plt.ylabel(ylabel)
    plt.grid(axis='y', linestyle='dashdot')
    plt.xticks(np.arange(start, end+1, 1))
    plt.title(title, y=-0.5, fontsize=12)
    plt.plot(x_smooth, scores_smooth, marker = mark, color = color, label = label, linestyle = linestyle)
...
plt.savefig("DPI_Evaluation_0.5_smooth.svg", bbox_inches = 'tight', dpi=80)
plt.show()